...
kableExtra::kbl(confusion_matrix, caption = "An example confusion matrix", format = "latex", escape = FALSE, align = "c") %>% 
...
```{r toy_example, fig.width=5, fig.height=2.5, fig.cap="\\label{fig:toy_example}Response curve example. The two panels are united by the same sequence of models used to construct the quality grid. The AUROC serves as the common reference scale on the x-axis."}
...
```{r gauss_RC, fig.width=6, fig.height=3, fig.cap="\\label{fig:gauss_RC}Mapping between AUROC and AUPRC for the binormal model."}
gauss_RC

```

```{r N10k_compare, fig.width=8, fig.height=2.5, fig.cap="\\label{fig:N10k_compare}Relative metric resolution by outcome prevalence and model quality for a binormal model with a sample size of N = 10,000. At each level of model quality 10,000 simulations are taken from the sampling model. Confidence limits are found from the .025 and .975 quantile values of the simulation samples."}
plot_relative

```

```{r SE_plot, fig.width=5, fig.height=3, fig.cap="\\label{fig:SE_plot}The relationship between AUROC and its standard error for different levels of outcome prevalence. The standard error is found using the approximation formula from @hanley1982meaning."}
SE_plot
...
```{r risk_distrib, fig.width=8, fig.height=4, fig.cap="\\label{fig:risk_distrib}Top panel: Estimated logit effects with kernel density estimates for the positive and negative class. Bottom panel: Q-Q norm plots of sample versus theoretical quantiles. Straight lines pass through the 1st and 3rd quartiles."}

ggarrange(plots = list(logit_distribution, qq_distribution), nrow=2)

```

```{r empirical_RC, fig.width=4, fig.height=2.5, fig.cap="\\label{fig:empirical_RC}An empirical response curve from the readmissions data logistic regression model. The black point shows baseline performance. The curve is constructed by incrementing positive scores above and below the baseline distribution."}
...
kableExtra::kbl(sim_results_summary, caption = "Simulation results summary. Lower and upper CI bounds are for 95 percent confidence intervals.", format = "latex", escape = FALSE, align = "c") %>% 
  kable_styling(latex_options = "hold_position") %>% 
  row_spec(0,bold=TRUE) %>% 
  column_spec(1, bold = TRUE)
...
```{r alt_resolution, fig.width=4, fig.height=2.5, fig.cap="\\label{fig:alt_resolution}Response curve plotting the AUROC references scale on the y-axis. A linear approximation of the response function at the evaluation point is shown in red."}
alt_plot
```
...
```{r N1k_compare, fig.width=8, fig.height=2.5, fig.cap="\\label{fig:N1k_compare}Relative resolution of AUPRC versus AUROC by outcome prevalence and model quality for a binormal model with a sample size of N = 1000. At each level of model quality 10,000 simulations are taken from the sampling model. Confidence limits are found from the .025 and .975 quantile values of the simulation samples."}
...
```{r N100k_compare, fig.width=8, fig.height=2.5, fig.cap="\\label{fig:N100k_compare}Relative metric resolution by outcome prevalence and model quality for a binormal model with a sample size of N = 100,000. At each level of model quality 10,000 simulations are taken from the sampling model. Confidence limits are found from the .025 and .975 quantile values of the simulation samples."}
